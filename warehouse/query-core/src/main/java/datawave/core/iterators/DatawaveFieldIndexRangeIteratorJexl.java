...
public class DatawaveFieldIndexRangeIteratorJexl extends DatawaveFieldIndexCachingIteratorJexl {
    ...
    private Key createLowerBoundKey(Text rowId, Text fiName, Text lowerBound, boolean isLowerInclusive) {
        ...
        if (isLowerInclusive) {
            ...
            if (compositeSeeker != null && compositeSeeker.getFieldToDiscreteIndexType().get(getFieldName().toString()) != null) {
                DiscreteIndexType discreteIndexType = compositeSeeker.getFieldToDiscreteIndexType().get(getFieldName().toString());
                this.boundingFiRangeStringBuilder.append(discreteIndexType.incrementIndex(lowerBound.toString()));
            } else {
