...
public class CompositeMetadataTest {
    ...
    public void setup() {
        ...
        for (String ingestType : ingestTypes) {
            for (String[] compFields : compositeFields)
                compositeMetadata.addCompositeFieldByType(ingestType, compFields[0], Arrays.asList(Arrays.copyOfRange(compFields, 1, compFields.length - 1)));
    ...
    public void filterCompositeMetadataTest() {
        Set<String> compFields = new HashSet<>();
        compFields.add("CAPTAIN_PLANET");
        compFields.add("GONNA_TAKE");
        ...
        CompositeMetadata fieldFilteredCompMetadata = compositeMetadata.filter(compFields);
        ...
        for (Multimap<String,String> compFieldMap : fieldFilteredCompMetadata.compositeFieldMapByType.values()) {
            Assert.assertTrue(compFieldMap.keySet().containsAll(compFields));
        ...
        CompositeMetadata filteredCompMetadata = compositeMetadata.filter(ingestTypes, compFields);
