...
/**
 * An implementation for {@link Auditor}, which writes URL encoded, JSON formatted audit messages to a file in HDFS. The dump auditor adds functionality which
 * will automatically close the open file handle after the specified amount of time has elapsed without receiving any new audit messages.
 */
public class DumpAuditor extends HdfsAuditor {
        ...
        if (lastUpdateTimer == null) {
            ...
            lastUpdateTimer.scheduleAtFixedRate(new TimerTask() {
                ...
                public void run() {
                    if (!auditsReceived) {
                        try {
                            log.info("Closing current audit dump file.");
                            close();
                        } catch (IOException e) {
            ...
            }, updateTimeoutMillis, updateTimeoutMillis);
        }
    ...
    public static class Builder extends HdfsAuditor.Builder<Builder> {
